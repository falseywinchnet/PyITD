#author: falsywinchnet, shameless stealer and modifier of matlab code
def segm_tec(f,N):
...
def EFD(x: list[numpy.float64], N: int):
    ...
    bounds[0] = 0 #fix : 5/10/22 temp patch for the first value
...
#for tomorrow: got the diff down considerably by fixing the omega first value to 0.
#got the diff down further by fixing the array mirroring to only add L exactly
